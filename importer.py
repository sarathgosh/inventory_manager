...
def parse_source_workbook(path: str | Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for row_number in range(4, sheet.max_row + 1):
        ...
        record = {
            ...
            "total_test_available": None,
            "total_test_done": None,
...
ALIASES = {
    ...
    "total_test_available": {
        "total test avail",
        "total test available",
        "total tests available",
        "tests available",
    },
    "total_test_done": {"total test done", "total tests done", "tests done"},
    ...
    "create_date": {"create date", "created date"},
    "opened_date": {"opened date", "date opened"},
    "expiry_date": {"expiry date", "expiration date", "expiry"},
    "finish_date": {"finish date", "finished date", "completion date"},
...
def parse_tabular_upload(filename: str, content: bytes) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for source_index, row in frame.iterrows():
        ...
        create_date = _date(source("create_date"))
        finish_date = _date(source("finish_date"))
        ...
        if create_date is None and _text(source("create_date")):
            notes.append(_source_note("create date", source("create_date")) or "")
        if finish_date is None and _text(source("finish_date")):
            notes.append(_source_note("finish date", source("finish_date")) or "")
        ...
        record = {
            ...
            "total_test_available": _number(source("total_test_available")),
            "total_test_done": _number(source("total_test_done")),
